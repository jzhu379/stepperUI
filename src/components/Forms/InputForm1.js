...
class InputForm1 extends Component {
  state = {
    parkNameError: false,
    parkAddressError: false,
  };

  getState = (id) => {
    return this.props.stateObj[id];
  };
  ...
  checkData = () => {
    if (
      this.props.stateObj["parkName"].trim().length === 0 ||
      this.props.stateObj["parkAddress"].trim().length === 0
    ) {
      if (this.props.stateObj["parkName"].trim().length === 0) {
        this.setState({ parkNameError: true });
      }
      if (this.props.stateObj["parkAddress"].trim().length === 0) {
        this.setState({ parkAddressError: true });
      }
    } else {
    ...
    return (
      <div className={classes.container}>
        ...
        <TextField
          ...
          helperText={
            this.state.parkAddressError
              ? "missing data"
              : "street, city, state, postal code"
          }
          onChange={(event) => {
            this.resetError("parkAddress");
            this.handleChange("parkAddress", event.target.value);
          }}
        />
